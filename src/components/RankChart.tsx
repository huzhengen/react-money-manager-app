type Props = {
  className?: string
  items?: { name: string; amount: number | string; sign: string }[]
}

const colors = ['#5470c6', '#ffbab0', '#ffa750', '#8748d3', '#53a867',
  '#eba953', '#91cc75', '#fac858', '#ee6666', '#73c0de']

export const RankChart: React.FC<Props> = (props) => {
  const { className, items } = props
  const total = items?.reduce((result, item) => result + parseFloat(item.amount.toString()), 0) ?? 0
  const max = items?.reduce((prev, item) => Math.max(prev, parseFloat(item.amount.toString())), 0) ?? 0
  const renderItems = () => {
    return (
      items?.map((item, index) =>
        <div key={item.name} grid grid-cols="[48px_1fr_1fr]" grid-rows="[repeat(2,1fr)]"
          ...
          <div row-start-1 col-start-2 row-end-2 col-end-3 self-end>
            {item.name} - {`${(parseFloat(item.amount.toString()) / total * 100).toFixed(0)}%`}
          </div>
          ...
          <div row-start-2 col-start-2 row-end-3 col-end-4 h-8px self-start
            ...
            <div absolute h-full rounded-4px
              style={{ background: colors[index], width: `${parseFloat(item.amount.toString()) / max * 100}%` }} />
          </div>
        </div>)
    )
  }
